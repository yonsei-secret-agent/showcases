...
4. manipulation check 추가: mode/check 복원 가능 여부
5. leakage audit는 enforcing으로 운영: regenerate/drop
6. judge mode 추가: diagnostic / decoupled
7. Judge B에 missing-check-type success criterion 제공
8. judge mode를 judgment record에 저장
9. source literal terms leakage audit 추가
10. condition × judge_mode summary 출력
11. paired case-level contrast 유지
12. deterministic post-processing으로 judge subfield consistency check
13. case-level clustered bootstrap 또는 paired permutation test 추가
14. experiment_version / prompt_hash 기반 output path 또는 resume key 사용
```
